"use strict";(()=>{var D=Object.defineProperty;var j=(t,n,e)=>n in t?D(t,n,{enumerable:!0,configurable:!0,writable:!0,value:e}):t[n]=e;var l=(t,n,e)=>j(t,typeof n!="symbol"?n+"":n,e);function u(){let t=globalThis,n=t.SpeechRecognition??t.webkitSpeechRecognition;if(!n)return null;let e=new n;e.continuous=!0,e.interimResults=!1,e.lang="en-US";let o=!1,a=[],r=[];return e.onresult=i=>{for(let c=0;c<i.results.length;c+=1){let h=i.results[c];if(h?.isFinal){let C=h[0]?.transcript?.trim();if(C)for(let F of a)F(C)}}},e.onerror=i=>{for(let c of r)c(String(i?.error??"unknown"))},e.onend=()=>{o=!1},{start:()=>{if(!o){o=!0;try{e.start()}catch{}}},stop:()=>{if(o){o=!1;try{e.stop()}catch{}}},onFinal:i=>{a.push(i)},onError:i=>{r.push(i)},isActive:()=>o}}function g(){let t=globalThis.speechSynthesis;if(!t)return{speak:async()=>{},cancel:()=>{},available:()=>!1};function n(){if(!t)return null;let e=t.getVoices();return e.find(o=>o.lang.startsWith("en-")&&o.default)??e.find(o=>o.lang.startsWith("en-"))??e[0]??null}return{speak:e=>new Promise(o=>{let a=new SpeechSynthesisUtterance(e),r=n();r&&(a.voice=r),a.rate=1,a.onend=()=>o(),a.onerror=()=>o(),t.speak(a)}),cancel:()=>t.cancel(),available:()=>!0}}function p(t,n){let e="idle",o=!1,a=[],r=i=>{if(e!==i){e=i;for(let c of a)c(i)}};return{start:()=>{if(e==="idle"){if(o){r("muted");return}t?.start(),r("listening")}},stop:()=>{t?.stop(),n.cancel(),r("idle")},speak:async i=>{e!=="idle"&&(t?.stop(),r("speaking"),await n.speak(i),o?r("muted"):(t?.start(),r("listening")))},setMuted:i=>{o=i,i?(t?.stop(),e==="listening"&&r("muted")):e==="muted"&&(t?.start(),r("listening"))},getState:()=>e,onStateChange:i=>{a.push(i)}}}var q="https://cdn.jsdelivr.net/npm",G="2.7.0";async function K(){return typeof globalThis.__SHOPPINGMATE_LIVEKIT_LOADER__=="function"?await globalThis.__SHOPPINGMATE_LIVEKIT_LOADER__():await import(`${q}/livekit-client@${G}/dist/livekit-client.esm.mjs`)}async function I(t){let n=await K(),e=new n.Room;return await e.connect(t.wsUrl,t.token),{setMicEnabled:o=>e.localParticipant.setMicrophoneEnabled(o),onData:o=>{e.on("dataReceived",a=>{a instanceof Uint8Array&&o(a)})},disconnect:()=>e.disconnect()}}function T(t){let n="idle",e=null,o=!1,a=[],r=i=>{if(n!==i){n=i;for(let c of a)c(i)}};return{start:()=>{n==="idle"&&(async()=>{try{e=await I({wsUrl:t.wsUrl,token:t.token,roomName:t.roomName}),e.onData(i=>t.onTranscriptEvent(i)),await e.setMicEnabled(!o),r(o?"muted":"listening")}catch(i){throw r("idle"),i}})().catch(i=>{console.warn("[voiceModeLiveKit] connect failed",i)})},stop:()=>{e?.disconnect().catch(()=>{}),e=null,r("idle")},speak:async()=>{},setMuted:i=>{o=i,e?.setMicEnabled(!i).catch(()=>{}),i?r("muted"):n==="muted"&&r("listening")},getState:()=>n,onStateChange:i=>{a.push(i)}}}function f(t){return t.stack==="web-speech"?p(u(),g()):t.stack==="live-kit"?t.livekit?T(t.livekit):(console.warn("[voiceModeFactory] live-kit stack requires livekit opts; returning null \u2192 caller falls back to chat"),null):null}async function E(t){try{let n=await fetch(`${t.apiBase}/v1/install`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({merchantId:t.merchantId,domain:t.domain,userAgent:navigator.userAgent,referrer:document.referrer||null})});if(!n.ok)return{kind:"err",reason:`install_${n.status}`};let e=await n.json(),o=await fetch(`${t.apiBase}/v1/session`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({merchantId:t.merchantId,domain:t.domain})});if(!o.ok)return{kind:"err",reason:`session_${o.status}`};let a=await o.json(),r=null;try{let i=await fetch(`${t.apiBase}/v1/voice/token`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({sessionId:a.sessionId,merchantId:t.merchantId})});i.ok?r=await i.json():console.warn("[shoppingmate] voice unavailable \u2014 status",i.status)}catch(i){console.warn("[shoppingmate] voice unavailable \u2014",i)}return{kind:"ok",sessionId:a.sessionId,wsUrl:a.wsUrl,merchantStatus:e.status,voice:r}}catch(n){return{kind:"err",reason:n instanceof Error?n.message:"network"}}}var A=0,m=()=>(A+=1,`t${A}`);function Y(t,n){switch(n.type){case"set_mode":return{...t,mode:n.mode};case"set_voice_state":return{...t,voiceState:n.state};case"set_connection":return{...t,connection:n.status};case"reset":return{...t,transcript:[],thinking:!1,closed:!1,closedReason:null,checkoutUrl:null,capWarning:null};case"user_input":return{...t,transcript:[...t.transcript,{id:m(),role:"user",kind:"text",text:n.text,ts:Date.now()}]};case"agent_event":{let e=n.event;switch(e.type){case"thinking":return{...t,thinking:!0};case"end_of_turn":return{...t,thinking:!1};case"say":return{...t,thinking:!1,transcript:[...t.transcript,{id:m(),role:"agent",kind:"text",text:e.text,ts:Date.now()}]};case"cards":return{...t,transcript:[...t.transcript,{id:m(),role:"agent",kind:"cards",items:e.items,ts:Date.now()}]};case"tool_result":return t;case"checkout_redirect":return{...t,checkoutUrl:e.url};case"cap_warning":return{...t,capWarning:{reason:e.reason,remaining:e.remaining},transcript:[...t.transcript,{id:m(),role:"system",kind:"cap_warning",remaining:e.remaining,ts:Date.now()}]};case"session_closed":return{...t,closed:!0,closedReason:e.reason,transcript:[...t.transcript,{id:m(),role:"system",kind:"closed",reason:e.reason,ts:Date.now()}]};default:return t}}default:return t}}function S(t){let n={sessionId:t.sessionId,mode:"pill",voiceState:"idle",transcript:[],thinking:!1,closed:!1,closedReason:null,checkoutUrl:null,capWarning:null,connection:"connecting"},e=[];return{get:()=>n,dispatch:o=>{n=Y(n,o);for(let a of e)a(n)},subscribe:o=>(e.push(o),()=>{let a=e.indexOf(o);a>=0&&e.splice(a,1)})}}var L=`
:host { all: initial; }
...
`;function b(t){return JSON.stringify(t)}function _(t){let n;try{n=JSON.parse(t)}catch{return null}if(!n||typeof n!="object")return null;let e=n;switch(e.type){case"thinking":return{type:"thinking"};case"say":return typeof e.text=="string"?{type:"say",text:e.text}:null;case"cards":return Array.isArray(e.items)?{type:"cards",items:e.items}:null;case"tool_result":return typeof e.toolName!="string"||typeof e.ok!="boolean"?null:{type:"tool_result",toolName:e.toolName,ok:e.ok,summary:typeof e.summary=="string"?e.summary:void 0};case"checkout_redirect":return typeof e.url=="string"?{type:"checkout_redirect",url:e.url}:null;case"cap_warning":return e.reason!=="turns"&&e.reason!=="voice_ms"&&e.reason!=="duration_ms"||typeof e.remaining!="number"?null:{type:"cap_warning",reason:e.reason,remaining:e.remaining};case"end_of_turn":return{type:"end_of_turn"};case"session_closed":return e.reason!=="user"&&e.reason!=="cap"&&e.reason!=="error"?null:{type:"session_closed",reason:e.reason};default:return null}}var N=[1e3,2e3,4e3,8e3,16e3],J=5;function H(t,n){let e=null,o=0,a=!1,r=[];function i(){a||(n.onStatus(o>0?"reconnecting":"connecting"),e=new WebSocket(t),e.onopen=()=>{n.onStatus("connected"),o>0&&e?.send(JSON.stringify({type:"session_resume",sessionId:n.sessionId})),o=0;for(let c of r)e?.send(c);r=[]},e.onmessage=c=>n.onEvent(typeof c.data=="string"?c.data:""),e.onerror=()=>{},e.onclose=()=>{if(a)return;if(o+=1,o>=J){n.onStatus("disconnected");return}let c=Math.min(o-1,N.length-1),h=N[c]??3e4;n.onStatus("reconnecting"),setTimeout(i,h)})}return i(),{send:c=>{e&&e.readyState===1?e.send(c):r.push(c)},close:()=>{a=!0,e?.close()}}}var s={pillCallable:"Talk to Sage",pillTextOnly:"Chat with Sage",pillCollapsed:"Sage",callBtn:"CALL",callBtnEnd:"END",chatBtnAria:"Open text chat",callBtnAria:"Start voice call",endCallAria:"End call",closeAria:"Close",callHeaderSpeaking:"speaking",callHeaderListening:"listening",chatHeaderSubtitle:"text fallback \xB7 voice preferred",chatPlaceholder:"Type a quick question\u2026",chatGreeting:"Hi, I'm Sage. What are you shopping for today?",reconnecting:"Reconnecting\u2026",disconnected:"Connection lost \u2014 reload to retry",closed:{user:"Conversation ended",cap:"Time to wrap up \u2014 reload for a new chat",error:"Something went wrong"},payNow:"Pay now \u2192",capWarning:"A couple minutes left",thinking:"Sage is thinking\u2026",micDenied:"Mic blocked \u2014 switching to text"};function v(t){return t.replace(/[&<>"']/g,n=>n==="&"?"&amp;":n==="<"?"&lt;":n===">"?"&gt;":n==='"'?"&quot;":"&#39;")}function X(t,n){let e=document.createElement("button");return e.className="card",e.type="button",e.dataset.sku=t.sku,e.innerHTML=`
  ...
  `,e.addEventListener("click",()=>n({sku:t.sku,variantId:t.variantId})),e}function x(t,n,e){t.innerHTML="";for(let o of n)if(o.kind==="text"){let a=document.createElement("div");a.className=`bubble ${o.role}`,a.textContent=o.text,t.appendChild(a)}else if(o.kind==="cards"){let a=document.createElement("div");a.className="cards-row";for(let r of o.items)a.appendChild(X(r,e));t.appendChild(a)}else if(o.kind==="cap_warning"){let a=document.createElement("div");a.className="bubble system",a.textContent=s.capWarning,t.appendChild(a)}else if(o.kind==="closed"){let a=document.createElement("div");a.className="bubble system",a.textContent=s.closed[o.reason],t.appendChild(a)}t.scrollTop=t.scrollHeight}var d=t=>`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${t}</svg>`,y=d('<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>'),O=d('<path d="M18 6 6 18"/><path d="m6 6 12 12"/>'),k=d('<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>'),w=d('<path d="M10.68 13.31a16 16 0 0 0 3.41 2.6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7 2 2 0 0 1 1.72 2v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.42 19.42 0 0 1-3.33-2.67"/><path d="M5 5a2 2 0 0 1 2-2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L10.9 11.1"/><line x1="22" y1="2" x2="2" y2="22"/>'),P=d('<path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="22"/>'),V=d('<line x1="2" y1="2" x2="22" y2="22"/><path d="M18.89 13.23A7.12 7.12 0 0 0 19 12v-2"/><path d="M5 10v2a7 7 0 0 0 12 5"/><path d="M15 9.34V5a3 3 0 0 0-5.68-1.33"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12"/><line x1="12" y1="19" x2="12" y2="22"/>'),$=d('<path d="M22 2 11 13"/><path d="M22 2l-7 20-4-9-9-4 20-7z"/>');function B(t,n){let e=n.voiceState==="speaking",o=n.muted?"you're muted":e?`Sage is ${s.callHeaderSpeaking}\u2026`:`${s.callHeaderListening} to you\u2026`;t.innerHTML=`
    <div class="panel">
      ...
      <div class="controls">
        <button class="ctrl ${n.muted?"muted":""}" data-action="mute" aria-pressed="${n.muted}" aria-label="${n.muted?"Unmute":"Mute"}">${n.muted?V:P}</button>
  ...
  `;let e=t.querySelector('[data-region="transcript"]');e instanceof HTMLElement&&x(e,n.transcript,n.onCardTap),t.querySelector('[data-action="call"]')?.addEventListener("click",n.onCall);let o=t.querySelector("form"),a=t.querySelector("input");o instanceof HTMLFormElement&&a instanceof HTMLInputElement&&o.addEventListener("submit",r=>{r.preventDefault();let i=a.value.trim();i&&(a.value="",n.onSend(i))})}function U(t,n){let e=n.mode!=="pill",o=n.mode==="call",a=n.mode==="chat",r=e?"Sage":n.callable?s.pillCallable:s.pillTextOnly;t.innerHTML=`
    <div class="pill" role="region" aria-label="Sage shopping assistant">
      ...
      ${e?`
        <div class="actions">
          ...
          <button class="btn btn-icon" data-action="chat" aria-pressed="${a}" aria-label="${s.chatBtnAria}">${y}</button>
          <button class="btn btn-icon" data-action="close" aria-label="${s.closeAria}">${O}</button>
        </div>
      `:""}
    </div>
  `,t.querySelector('[data-action="toggle"]')?.addEventListener("click",()=>{n.mode==="pill"?n.onChat():n.onClose()}),t.querySelector('[data-action="call"]')?.addEventListener("click",n.onCall),t.querySelector('[data-action="chat"]')?.addEventListener("click",n.onChat),t.querySelector('[data-action="close"]')?.addEventListener("click",n.onClose)}var W="shoppingmate-widget";function Q(){return"live-kit"==="web-speech"?"web-speech":"live-kit"}var M=class extends HTMLElement{constructor(){super(...arguments);l(this,"rootEl",null);l(this,"pillHost",null);l(this,"panelHost",null);l(this,"store",S({sessionId:"pending"}));l(this,"socket",null);l(this,"voiceMode",p(null,g()));l(this,"voice",null);l(this,"apiBase","");l(this,"merchantId","");l(this,"domain",window.location.host)}connectedCallback(){if(this.shadowRoot)return;let e=this.getAttribute("data-id"),o=this.getAttribute("data-api")??this.apiBase;if(!e){console.warn("[shoppingmate] data-id missing on widget element");return}this.merchantId=e,this.apiBase=o;let a=this.attachShadow({mode:"open"}),r=document.createElement("style");r.textContent=L,a.appendChild(r);let i=document.createElement("div");i.className="root",a.appendChild(i),this.rootEl=i,this.panelHost=document.createElement("div"),this.pillHost=document.createElement("div"),i.appendChild(this.panelHost),i.appendChild(this.pillHost),this.store.subscribe(()=>this.render()),this.render(),this.start()}disconnectedCallback(){this.socket?.close(),this.voiceMode.stop()}async start(){let e=await E({apiBase:this.apiBase,merchantId:this.merchantId,domain:this.domain});if(e.kind==="err"){console.warn("[shoppingmate] bootstrap failed:",e.reason);return}this.store=S({sessionId:e.sessionId}),this.store.subscribe(()=>this.render()),this.voice=e.voice;let o=Q(),a=u();if(o==="live-kit"&&this.voice){let r=f({stack:"live-kit",livekit:{wsUrl:this.voice.wsUrl,token:this.voice.token,roomName:this.voice.roomName,onTranscriptEvent:i=>this.handleLiveKitData(i)}});r&&(this.voiceMode=r)}else{let r=f({stack:"web-speech"});r&&(this.voiceMode=r),a?.onFinal(i=>{this.store.dispatch({type:"user_input",text:i,mode:"voice"}),this.socket?.send(b({type:"user_text",sessionId:e.sessionId,text:i,mode:"voice"}))})}this.voiceMode.onStateChange(r=>this.store.dispatch({type:"set_voice_state",state:r})),this.socket=H(e.wsUrl,{sessionId:e.sessionId,onEvent:r=>{let i=_(r);i&&(this.store.dispatch({type:"agent_event",event:i}),i.type==="say"&&this.voiceMode.speak(i.text))},onStatus:r=>this.store.dispatch({type:"set_connection",status:r})})}render(){if(!this.pillHost||!this.panelHost)return;let e=this.store.get(),o=u()!==null;e.mode==="call"?B(this.panelHost,{voiceState:e.voiceState,muted:e.voiceState==="muted",transcript:e.transcript,checkoutUrl:e.checkoutUrl,onMute:a=>this.voiceMode.setMuted(a),onEnd:()=>{this.voiceMode.stop(),this.store.dispatch({type:"set_mode",mode:"expanded"})},onChat:()=>this.store.dispatch({type:"set_mode",mode:"chat"}),onCardTap:a=>this.cardTap(a),onCheckout:()=>{}}):e.mode==="chat"?R(this.panelHost,{transcript:e.transcript,checkoutUrl:e.checkoutUrl,onSend:a=>this.userText(a,"text"),onCall:()=>this.openCall(),onCardTap:a=>this.cardTap(a),closed:e.closed}):this.panelHost.innerHTML="",U(this.pillHost,{mode:e.mode,callable:o,onCall:()=>this.openCall(),onChat:()=>this.store.dispatch({type:"set_mode",mode:"chat"}),onClose:()=>this.store.dispatch({type:"set_mode",mode:"pill"})})}openCall(){this.store.dispatch({type:"set_mode",mode:"call"}),this.voiceMode.start()}userText(e,o){this.store.dispatch({type:"user_input",text:e,mode:o});let a=this.store.get().sessionId;this.socket?.send(b({type:"user_text",sessionId:a,text:e,mode:o}))}handleLiveKitData(e){let o;try{o=new TextDecoder().decode(e)}catch{return}let a=_(o);a&&this.store.dispatch({type:"agent_event",event:a})}cardTap(e){let o=this.store.get().sessionId;this.socket?.send(b({type:"card_tap",sessionId:o,action:"cartAdd",sku:e.sku,variantId:e.variantId,qty:1}))}};function z(){customElements.get(W)||customElements.define(W,M)}function Z(){let t=document.currentScript instanceof HTMLScriptElement?document.currentScript:null,n=t?.dataset.id;if(!n){console.warn("[shoppingmate] data-id missing on script tag");return}if(document.querySelector("shoppingmate-widget"))return;z();let e=document.createElement("shoppingmate-widget");e.setAttribute("data-id",n);let o=t?.dataset.api;e.setAttribute("data-api",o??"https://api.shoppingmate.ai"),document.body?document.body.appendChild(e):document.addEventListener("DOMContentLoaded",()=>document.body.appendChild(e),{once:!0})}Z();})();
