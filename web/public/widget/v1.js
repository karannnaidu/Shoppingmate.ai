"use strict";(()=>{var fe=Object.defineProperty;var he=(e,n,t)=>n in e?fe(e,n,{enumerable:!0,configurable:!0,writable:!0,value:t}):e[n]=t;var u=(e,n,t)=>he(e,typeof n!="symbol"?n+"":n,t);function b(){let e=globalThis,n=e.SpeechRecognition??e.webkitSpeechRecognition;if(!n)return null;let t=new n;t.continuous=!0,t.interimResults=!1,t.lang="en-US";let o=!1,i=[],a=[];return t.onresult=r=>{for(let s=0;s<r.results.length;s+=1){let c=r.results[s];if(c?.isFinal){let l=c[0]?.transcript?.trim();if(l)for(let p of i)p(l)}}},t.onerror=r=>{for(let s of a)s(String(r?.error??"unknown"))},t.onend=()=>{o=!1},{start:()=>{if(!o){o=!0;try{t.start()}catch{}}},stop:()=>{if(o){o=!1;try{t.stop()}catch{}}},onFinal:r=>{i.push(r)},onError:r=>{a.push(r)},isActive:()=>o}}function k(){let e=globalThis.speechSynthesis;if(!e)return{speak:async()=>{},cancel:()=>{},available:()=>!1};function n(){if(!e)return null;let t=e.getVoices();return t.find(o=>o.lang.startsWith("en-")&&o.default)??t.find(o=>o.lang.startsWith("en-"))??t[0]??null}return{speak:t=>new Promise(o=>{let i=new SpeechSynthesisUtterance(t),a=n();a&&(i.voice=a),i.rate=1,i.onend=()=>o(),i.onerror=()=>o(),e.speak(i)}),cancel:()=>e.cancel(),available:()=>!0}}function h(e,n){let t="idle",o=!1,i=[],a=r=>{if(t!==r){t=r;for(let s of i)s(r)}};return{start:()=>{if(t==="idle"){if(o){a("muted");return}e?.start(),a("listening")}},stop:()=>{e?.stop(),n.cancel(),a("idle")},speak:async r=>{t!=="idle"&&(e?.stop(),a("speaking"),await n.speak(r),o?a("muted"):(e?.start(),a("listening")))},setMuted:r=>{o=r,r?(e?.stop(),t==="listening"&&a("muted")):t==="muted"&&(e?.start(),a("listening"))},getState:()=>t,onStateChange:r=>{i.push(r)}}}var ye="https://cdn.jsdelivr.net/npm",be="2.7.0";async function ve(){return typeof globalThis.__SHOPPINGMATE_LIVEKIT_LOADER__=="function"?await globalThis.__SHOPPINGMATE_LIVEKIT_LOADER__():await import(`${ye}/livekit-client@${be}/dist/livekit-client.esm.mjs`)}async function D(e){let n=await ve(),t=new n.Room,o=new Map;t.on("trackSubscribed",a=>{let r=a;if(r.kind!=="audio")return;let s=r.attach();s.style.display="none",document.body.appendChild(s),o.set(a,s)}),t.on("trackUnsubscribed",a=>{let r=o.get(a);r&&(r.remove(),o.delete(a)),a.detach?.()});let i=[];return t.on("activeSpeakersChanged",a=>{let s=(a??[]).some(c=>!c.isLocal);for(let c of i)c(s)}),await t.connect(e.wsUrl,e.token),{setMicEnabled:a=>t.localParticipant.setMicrophoneEnabled(a),onData:a=>{t.on("dataReceived",r=>{r instanceof Uint8Array&&a(r)})},onAgentSpeaking:a=>{i.push(a)},publishData:a=>t.localParticipant.publishData(a,{reliable:!0}),disconnect:async()=>{for(let a of o.values())a.remove();o.clear(),await t.disconnect()}}}function O(e){let n="idle",t=null,o=!1,i=[],a=r=>{if(n!==r){n=r;for(let s of i)s(r)}};return{start:()=>{n==="idle"&&(a("connecting"),(async()=>{try{t=await D({wsUrl:e.wsUrl,token:e.token,roomName:e.roomName}),t.onData(r=>e.onTranscriptEvent(r)),t.onAgentSpeaking(r=>{o||a(r?"speaking":"listening")}),await t.setMicEnabled(!o),a(o?"muted":"listening")}catch(r){throw a("idle"),r}})().catch(r=>{console.warn("[voiceModeLiveKit] connect failed",r)}))},stop:()=>{t?.disconnect().catch(()=>{}),t=null,a("idle")},speak:async()=>{},setMuted:r=>{o=r,t?.setMicEnabled(!r).catch(()=>{}),r?a("muted"):n==="muted"&&a("listening")},getState:()=>n,onStateChange:r=>{i.push(r)},publishData:async r=>{t&&await t.publishData(r)}}}function w(e){return e.stack==="web-speech"?h(b(),k()):e.stack==="live-kit"?e.livekit?O(e.livekit):(console.warn("[voiceModeFactory] live-kit stack requires livekit opts; returning null \u2192 caller falls back to chat"),null):null}async function U(e){try{let n=await fetch(`${e.apiBase}/v1/install`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({merchantId:e.merchantId,domain:e.domain,userAgent:navigator.userAgent,referrer:document.referrer||null})});if(!n.ok)return{kind:"err",reason:`install_${n.status}`};let t=await n.json(),o=await fetch(`${e.apiBase}/v1/session`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({merchantId:e.merchantId,domain:e.domain})});if(!o.ok)return{kind:"err",reason:`session_${o.status}`};let i=await o.json(),a=null;try{let r=await fetch(`${e.apiBase}/v1/voice/token`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({sessionId:i.sessionId,merchantId:e.merchantId})});r.ok?a=await r.json():console.warn("[shoppingmate] voice unavailable \u2014 status",r.status)}catch(r){console.warn("[shoppingmate] voice unavailable \u2014",r)}return{kind:"ok",sessionId:i.sessionId,wsUrl:i.wsUrl,merchantStatus:t.status,personaId:t.personaId??a?.personaId??null,voice:a}}catch(n){return{kind:"err",reason:n instanceof Error?n.message:"network"}}}function V(e){let n=0,t=r=>{let s=Date.now();s-n<200||(n=s,e.send(r))},o=r=>{let s=r.target;if(!s)return;let c=xe(s),l=c?ke(c,e.hints):null;t({type:"visitor_action",sessionId:e.sessionId,action:"click",intentKey:l,url:window.location.href,elementLabel:c,timestamp:Date.now()})},i=()=>{t({type:"visitor_action",sessionId:e.sessionId,action:"route_change",intentKey:null,url:window.location.href,elementLabel:null,timestamp:Date.now()})},a=r=>{let s=r.target;if(!s)return;let c=s.tagName?.toLowerCase();c!=="input"&&c!=="textarea"&&c!=="select"||s.type==="password"||t({type:"visitor_action",sessionId:e.sessionId,action:"form_focus",intentKey:null,url:window.location.href,elementLabel:s.name||s.id||null,timestamp:Date.now()})};return document.addEventListener("click",o,{passive:!0,capture:!0}),window.addEventListener("popstate",i),document.addEventListener("focusin",a,{passive:!0}),()=>{document.removeEventListener("click",o,!0),window.removeEventListener("popstate",i),document.removeEventListener("focusin",a)}}function xe(e){return e.getAttribute("aria-label")??e.getAttribute("title")??(e.textContent??"").trim().slice(0,80)??null}function ke(e,n){let t=e.toLowerCase();if(n.has(t))return t;for(let o of n.keys())if(t.includes(o)||o.includes(t))return o;return null}var we=new Set(["the","a","an","to","of","on","in","and","or","section","button","link","card","tile","now"]),Se=[{keyword:"button",matchTag:/^(button)$/i,matchRole:"button"},{keyword:"link",matchTag:/^(a)$/i,matchRole:"link"},{keyword:"card",matchTag:/^(article|div|section)$/i},{keyword:"section",matchTag:/^(section|main|article)$/i}],_e=.4;function S(e,n){if(n){let a=n.get(e.toLowerCase().trim());if(a)try{let r=document.querySelector(a);if(r instanceof HTMLElement&&B(r))return r}catch{}}let t=A(e);if(t.size===0)return null;let o=Te(document.body),i=null;for(let a of o){if(!a.visible)continue;let r=Ee(a,e,t);r<_e||(!i||r>i.score)&&(i={c:a,score:r})}return i?.c.element??null}function A(e){return new Set(e.toLowerCase().replace(/[^a-z0-9 ]/g," ").split(/\s+/).filter(n=>n.length>0&&!we.has(n)))}function Te(e){let n=[],t=document.createTreeWalker(e,NodeFilter.SHOW_ELEMENT),o=t.nextNode();for(;o;){if(o instanceof HTMLElement){let i=Me(o);i&&n.push({element:o,role:o.getAttribute("role")??o.tagName.toLowerCase(),name:i,visible:B(o)})}o=t.nextNode()}return n}function Me(e){let n=e.getAttribute("aria-labelledby");if(n){let a=n.split(/\s+/).map(r=>document.getElementById(r)?.textContent?.trim()??"").filter(Boolean);if(a.length>0)return a.join(" ")}let t=e.getAttribute("aria-label");if(t)return t.trim();if(e.id){let a=document.querySelector(`label[for="${Ce(e.id)}"]`);if(a?.textContent)return a.textContent.trim()}let o=e.getAttribute("alt")??e.getAttribute("title");if(o)return o.trim();let i=(e.textContent??"").trim();return i&&i.length<200?i:""}function B(e){if(!e.isConnected)return!1;let n=e.ownerDocument.defaultView?.getComputedStyle(e);return n?!(n.display==="none"||n.visibility==="hidden"||n.opacity==="0"):!0}function Ee(e,n,t){let o=A(e.name);if(o.size===0)return 0;let i=0;for(let m of t)o.has(m)&&i++;let a=new Set([...t,...o]).size,r=a===0?0:i/a,s=0,c=n.toLowerCase();for(let m of Se)if(c.includes(m.keyword)&&(m.matchTag.test(e.element.tagName)||e.role===m.matchRole)){s=.15;break}let l=e.element.getAttribute("data-tour-stop"),p=0;if(l){let m=A(l.replace(/-/g," ")),y=0;for(let I of t)m.has(I)&&y++;y>0&&(p=.5*(y/t.size))}return Math.min(1,r+s+p)}function Ce(e){return e.replace(/(["\\])/g,"\\$1")}var Ie="data-shoppingmate-bot-cursor",W="data-shoppingmate-cursor-keyframes",v=null,L=window.innerWidth-80,H=window.innerHeight-80;function z(){if(v&&v.isConnected)return v;Ae();let e=document.createElement("div");return e.setAttribute(Ie,""),e.innerHTML=`
...
`;function We(e){if(!e||typeof e.type!="string")return!1;switch(e.type){case"navigate":return typeof e.path=="string";case"scroll_to":case"highlight":case"click":case"point_at":case"demo_click":return typeof e.intent=="string";default:return!1}}function x(e){return JSON.stringify(e)}function $(e){let n;try{n=JSON.parse(e)}catch{return null}if(!n||typeof n!="object")return null;let t=n;switch(t.type){case"thinking":return{type:"thinking"};case"say":return typeof t.text=="string"?{type:"say",text:t.text}:null;case"say_partial":return typeof t.text=="string"?{type:"say_partial",text:t.text}:null;case"user_text":return typeof t.text=="string"?{type:"user_text",text:t.text}:null;case"cards":return Array.isArray(t.items)?{type:"cards",items:t.items}:null;case"tool_result":return typeof t.toolName!="string"||typeof t.ok!="boolean"?null:{type:"tool_result",toolName:t.toolName,ok:t.ok,summary:typeof t.summary=="string"?t.summary:void 0};case"checkout_redirect":return typeof t.url=="string"?{type:"checkout_redirect",url:t.url}:null;case"cap_warning":return t.reason!=="turns"&&t.reason!=="voice_ms"&&t.reason!=="duration_ms"||typeof t.remaining!="number"?null:{type:"cap_warning",reason:t.reason,remaining:t.remaining};case"end_of_turn":return{type:"end_of_turn"};case"session_closed":return t.reason!=="user"&&t.reason!=="cap"&&t.reason!=="error"?null:{type:"session_closed",reason:t.reason};case"host_action_request":{if(typeof t.callId!="string"||!t.action)return null;let o=t.action;return We(o)?{type:"host_action_request",callId:t.callId,action:o}:null}case"persona_swap":return typeof t.personaId=="string"?{type:"persona_swap",personaId:t.personaId}:null;default:return null}}var Q=[1e3,2e3,4e3,8e3,16e3],ze=5;function ee(e,n){let t=null,o=0,i=!1,a=[];function r(){i||(n.onStatus(o>0?"reconnecting":"connecting"),t=new WebSocket(e),t.onopen=()=>{n.onStatus("connected"),o>0&&t?.send(JSON.stringify({type:"session_resume",sessionId:n.sessionId})),o=0;for(let s of a)t?.send(s);a=[]},t.onmessage=s=>n.onEvent(typeof s.data=="string"?s.data:""),t.onerror=()=>{},t.onclose=()=>{if(i)return;if(o+=1,o>=ze){n.onStatus("disconnected");return}let s=Math.min(o-1,Q.length-1),c=Q[s]??3e4;n.onStatus("reconnecting"),setTimeout(r,c)})}return r(),{send:s=>{t&&t.readyState===1?t.send(s):a.push(s)},close:()=>{i=!0,t?.close()}}}var d={trayConnected:"CONNECTED",trayConnecting:"CONNECTING\u2026",trayOffline:"OFFLINE",micStart:"Start voice call",micMute:"Mute mic",micUnmute:"Unmute mic",endCallAria:"End call",closeAria:"Close",openAria:"Open shoppingmate",panelHelpHeading:"Hi, I'm",panelHelpSubtitle:"I'm here to help you:",panelBullets:["Find the right product fast","Compare options out loud","Check out without leaving the page"],poweredBy:"Powered by shoppingmate",chatPlaceholder:"Type a quick question\u2026",reconnecting:"Reconnecting\u2026",disconnected:"Connection lost \u2014 reload to retry",closed:{user:"Conversation ended",cap:"Time to wrap up \u2014 reload for a new chat",error:"Something went wrong"},payNow:"Pay now \u2192",capWarning:"A couple minutes left",thinking:"thinking\u2026",micDenied:"Mic blocked \u2014 switching to text"};var f=e=>`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${e}</svg>`,It=f('<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>'),M=f('<path d="M18 6 6 18"/><path d="m6 6 12 12"/>'),At=f('<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>'),te=f('<path d="M10.68 13.31a16 16 0 0 0 3.41 2.6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7 2 2 0 0 1 1.72 2v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.42 19.42 0 0 1-3.33-2.67"/><path d="M5 5a2 2 0 0 1 2-2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L10.9 11.1"/><line x1="22" y1="2" x2="2" y2="22"/>'),ne=f('<path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="22"/>'),oe=f('<line x1="2" y1="2" x2="22" y2="22"/><path d="M18.89 13.23A7.12 7.12 0 0 0 19 12v-2"/><path d="M5 10v2a7 7 0 0 0 12 5"/><path d="M15 9.34V5a3 3 0 0 0-5.68-1.33"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12"/><line x1="12" y1="19" x2="12" y2="22"/>'),re=f('<path d="M22 2 11 13"/><path d="M22 2l-7 20-4-9-9-4 20-7z"/>');function E(e){return e.replace(/[&<>"']/g,n=>n==="&"?"&amp;":n==="<"?"&lt;":n===">"?"&gt;":n==='"'?"&quot;":"&#39;")}function je(e,n){let t=document.createElement("button");return t.className="card",t.type="button",t.dataset.sku=e.sku,t.innerHTML=`
  ...
  `,I=n.callable?o?i?d.micUnmute:d.micMute:d.micStart:d.micStart,me=i?oe:ne,ge=!o;e.innerHTML=`
    <div class="tray" role="region" aria-label="shoppingmate">
      ...
      <div class="tray-controls">
        <button class="tray-btn ${i?"muted":""}" data-action="mic" aria-pressed="${i}" aria-label="${I}">${me}</button>
  ...
  `,o.querySelector('[data-action="accept"]')?.addEventListener("click",n),o.querySelector('[data-action="dismiss"]')?.addEventListener("click",t),e.appendChild(o),o}var pe="shoppingmate-widget";function Xe(){return"live-kit"==="web-speech"?"web-speech":"live-kit"}var R=class extends HTMLElement{constructor(){super(...arguments);u(this,"rootEl",null);u(this,"pillHost",null);u(this,"panelHost",null);u(this,"store",P({sessionId:"pending"}));u(this,"socket",null);u(this,"voiceMode",h(null,k()));u(this,"voice",null);u(this,"persona",Y());u(this,"apiBase","");u(this,"merchantId","");u(this,"domain",window.location.host);u(this,"stopActivityTracker",null)}connectedCallback(){if(this.shadowRoot)return;let t=this.getAttribute("data-id"),o=this.getAttribute("data-api")??this.apiBase;if(!t){console.warn("[shoppingmate] data-id missing on widget element");return}this.merchantId=t,this.apiBase=o;let i=this.attachShadow({mode:"open"}),a=document.createElement("style");a.textContent=Z,i.appendChild(a);let r=document.createElement("div");r.className="root",i.appendChild(r),this.rootEl=r,this.panelHost=document.createElement("div"),this.pillHost=document.createElement("div"),r.appendChild(this.panelHost),r.appendChild(this.pillHost),this.store.subscribe(()=>this.render()),this.render(),this.start()}disconnectedCallback(){this.socket?.close(),this.voiceMode.stop(),this.stopActivityTracker?.()}async start(){let t=await U({apiBase:this.apiBase,merchantId:this.merchantId,domain:this.domain});if(t.kind==="err"){console.warn("[shoppingmate] bootstrap failed:",t.reason);return}this.store=P({sessionId:t.sessionId}),this.store.subscribe(()=>this.render()),this.voice=t.voice,this.persona=J(t.personaId??t.voice?.personaId??null);let o=Xe(),i=b();if(o==="live-kit"&&this.voice){let r=w({stack:"live-kit",livekit:{wsUrl:this.voice.wsUrl,token:this.voice.token,roomName:this.voice.roomName,onTranscriptEvent:s=>this.handleLiveKitData(s)}});r&&(this.voiceMode=r)}else{let r=w({stack:"web-speech"});r&&(this.voiceMode=r),i?.onFinal(s=>{this.store.dispatch({type:"user_input",text:s,mode:"voice"}),this.socket?.send(x({type:"user_text",sessionId:t.sessionId,text:s,mode:"voice"}))})}this.voiceMode.onStateChange(r=>this.store.dispatch({type:"set_voice_state",state:r})),this.socket=ee(t.wsUrl,{sessionId:t.sessionId,onEvent:r=>{let s=$(r);s&&this.handleAgentEvent(s)},onStatus:r=>this.store.dispatch({type:"set_connection",status:r})}),this.merchantId==="SM-XPK2EN"&&de(document.body,{onAccept:()=>{this.publishWidgetMessage({type:"tour_request"}),this.openCall()},onDismiss:()=>{}}),this.stopActivityTracker=V({sessionId:t.sessionId,hints:new Map,send:r=>this.publishWidgetMessage(r)})}async handleAgentEvent(t,o="ws"){if(t.type==="host_action_request"){let i=await K(t.action);this.publishWidgetMessage({type:"host_action_result",callId:t.callId,result:i},o);return}t.type!=="persona_swap"&&(this.store.dispatch({type:"agent_event",event:t}),t.type==="say"&&this.voiceMode.speak(t.text))}publishWidgetMessage(t,o="ws"){let i=x(t);if(o==="livekit"&&this.voiceMode.publishData){let a=new TextEncoder().encode(i);this.voiceMode.publishData(a);return}this.socket?.send(i)}render(){if(!this.pillHost||!this.panelHost)return;let t=this.store.get(),o=b()!==null;t.mode==="call"?se(this.panelHost,{voiceState:t.voiceState,muted:t.voiceState==="muted",transcript:t.transcript,checkoutUrl:t.checkoutUrl,personaName:this.persona.name,onClose:()=>this.store.dispatch({type:"set_mode",mode:"pill"}),onCardTap:i=>this.cardTap(i),onCheckout:()=>{}}):t.mode==="chat"||t.mode==="expanded"?ce(this.panelHost,{transcript:t.transcript,checkoutUrl:t.checkoutUrl,personaName:this.persona.name,personaInitial:this.persona.initial,personaAvatarUrl:this.persona.avatarUrl,onSend:i=>this.userText(i,"text"),onCall:()=>this.openCall(),onClose:()=>this.store.dispatch({type:"set_mode",mode:"pill"}),onCardTap:i=>this.cardTap(i),closed:t.closed}):this.panelHost.innerHTML="",le(this.pillHost,{mode:t.mode,callable:o,voiceState:t.voiceState,personaName:this.persona.name,personaInitial:this.persona.initial,personaAvatarUrl:this.persona.avatarUrl,onCall:()=>this.openCall(),onMute:i=>this.voiceMode.setMuted(i),onEnd:()=>{this.voiceMode.stop(),this.store.dispatch({type:"set_mode",mode:"pill"})},onChat:()=>this.store.dispatch({type:"set_mode",mode:"chat"}),onClose:()=>this.store.dispatch({type:"set_mode",mode:"pill"})})}openCall(){this.store.dispatch({type:"set_mode",mode:"call"}),this.voiceMode.start()}userText(t,o){this.store.dispatch({type:"user_input",text:t,mode:o});let i=this.store.get().sessionId;this.socket?.send(x({type:"user_text",sessionId:i,text:t,mode:o}))}handleLiveKitData(t){let o;try{o=new TextDecoder().decode(t)}catch{return}let i=$(o);i&&this.handleAgentEvent(i,"livekit")}cardTap(t){let o=this.store.get().sessionId;this.socket?.send(x({type:"card_tap",sessionId:o,action:"cartAdd",sku:t.sku,variantId:t.variantId,qty:1}))}};function ue(){customElements.get(pe)||customElements.define(pe,R)}function Ze(){let e=document.currentScript instanceof HTMLScriptElement?document.currentScript:null,n=e?.dataset.id;if(!n){console.warn("[shoppingmate] data-id missing on script tag");return}if(document.querySelector("shoppingmate-widget"))return;ue();let t=document.createElement("shoppingmate-widget");t.setAttribute("data-id",n);let o=e?.dataset.api;t.setAttribute("data-api",o??"https://api-production-1ea1.up.railway.app"),document.body?document.body.appendChild(t):document.addEventListener("DOMContentLoaded",()=>document.body.appendChild(t),{once:!0})}Ze();})();
