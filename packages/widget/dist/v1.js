"use strict";(()=>{var ne=Object.defineProperty;var oe=(e,n,t)=>n in e?ne(e,n,{enumerable:!0,configurable:!0,writable:!0,value:t}):e[n]=t;var m=(e,n,t)=>oe(e,typeof n!="symbol"?n+"":n,t);function b(){let e=globalThis,n=e.SpeechRecognition??e.webkitSpeechRecognition;if(!n)return null;let t=new n;t.continuous=!0,t.interimResults=!1,t.lang="en-US";let o=!1,a=[],i=[];return t.onresult=r=>{for(let s=0;s<r.results.length;s+=1){let c=r.results[s];if(c?.isFinal){let d=c[0]?.transcript?.trim();if(d)for(let p of a)p(d)}}},t.onerror=r=>{for(let s of i)s(String(r?.error??"unknown"))},t.onend=()=>{o=!1},{start:()=>{if(!o){o=!0;try{t.start()}catch{}}},stop:()=>{if(o){o=!1;try{t.stop()}catch{}}},onFinal:r=>{a.push(r)},onError:r=>{i.push(r)},isActive:()=>o}}function x(){let e=globalThis.speechSynthesis;if(!e)return{speak:async()=>{},cancel:()=>{},available:()=>!1};function n(){if(!e)return null;let t=e.getVoices();return t.find(o=>o.lang.startsWith("en-")&&o.default)??t.find(o=>o.lang.startsWith("en-"))??t[0]??null}return{speak:t=>new Promise(o=>{let a=new SpeechSynthesisUtterance(t),i=n();i&&(a.voice=i),a.rate=1,a.onend=()=>o(),a.onerror=()=>o(),e.speak(a)}),cancel:()=>e.cancel(),available:()=>!0}}function h(e,n){let t="idle",o=!1,a=[],i=r=>{if(t!==r){t=r;for(let s of a)s(r)}};return{start:()=>{if(t==="idle"){if(o){i("muted");return}e?.start(),i("listening")}},stop:()=>{e?.stop(),n.cancel(),i("idle")},speak:async r=>{t!=="idle"&&(e?.stop(),i("speaking"),await n.speak(r),o?i("muted"):(e?.start(),i("listening")))},setMuted:r=>{o=r,r?(e?.stop(),t==="listening"&&i("muted")):t==="muted"&&(e?.start(),i("listening"))},getState:()=>t,onStateChange:r=>{a.push(r)}}}var re="https://cdn.jsdelivr.net/npm",ae="2.7.0";async function ie(){return typeof globalThis.__SHOPPINGMATE_LIVEKIT_LOADER__=="function"?await globalThis.__SHOPPINGMATE_LIVEKIT_LOADER__():await import(`${re}/livekit-client@${ae}/dist/livekit-client.esm.mjs`)}async function L(e){let n=await ie(),t=new n.Room,o=new Map;t.on("trackSubscribed",i=>{let r=i;if(r.kind!=="audio")return;let s=r.attach();s.style.display="none",document.body.appendChild(s),o.set(i,s)}),t.on("trackUnsubscribed",i=>{let r=o.get(i);r&&(r.remove(),o.delete(i)),i.detach?.()});let a=[];return t.on("activeSpeakersChanged",i=>{let s=(i??[]).some(c=>!c.isLocal);for(let c of a)c(s)}),await t.connect(e.wsUrl,e.token),{setMicEnabled:i=>t.localParticipant.setMicrophoneEnabled(i),onData:i=>{t.on("dataReceived",r=>{r instanceof Uint8Array&&i(r)})},onAgentSpeaking:i=>{a.push(i)},disconnect:async()=>{for(let i of o.values())i.remove();o.clear(),await t.disconnect()}}}function N(e){let n="idle",t=null,o=!1,a=[],i=r=>{if(n!==r){n=r;for(let s of a)s(r)}};return{start:()=>{n==="idle"&&(async()=>{try{t=await L({wsUrl:e.wsUrl,token:e.token,roomName:e.roomName}),t.onData(r=>e.onTranscriptEvent(r)),t.onAgentSpeaking(r=>{o||i(r?"speaking":"listening")}),await t.setMicEnabled(!o),i(o?"muted":"listening")}catch(r){throw i("idle"),r}})().catch(r=>{console.warn("[voiceModeLiveKit] connect failed",r)})},stop:()=>{t?.disconnect().catch(()=>{}),t=null,i("idle")},speak:async()=>{},setMuted:r=>{o=r,t?.setMicEnabled(!r).catch(()=>{}),r?i("muted"):n==="muted"&&i("listening")},getState:()=>n,onStateChange:r=>{a.push(r)}}}function k(e){return e.stack==="web-speech"?h(b(),x()):e.stack==="live-kit"?e.livekit?N(e.livekit):(console.warn("[voiceModeFactory] live-kit stack requires livekit opts; returning null \u2192 caller falls back to chat"),null):null}async function $(e){try{let n=await fetch(`${e.apiBase}/v1/install`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({merchantId:e.merchantId,domain:e.domain,userAgent:navigator.userAgent,referrer:document.referrer||null})});if(!n.ok)return{kind:"err",reason:`install_${n.status}`};let t=await n.json(),o=await fetch(`${e.apiBase}/v1/session`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({merchantId:e.merchantId,domain:e.domain})});if(!o.ok)return{kind:"err",reason:`session_${o.status}`};let a=await o.json(),i=null;try{let r=await fetch(`${e.apiBase}/v1/voice/token`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({sessionId:a.sessionId,merchantId:e.merchantId})});r.ok?i=await r.json():console.warn("[shoppingmate] voice unavailable \u2014 status",r.status)}catch(r){console.warn("[shoppingmate] voice unavailable \u2014",r)}return{kind:"ok",sessionId:a.sessionId,wsUrl:a.wsUrl,merchantStatus:t.status,voice:i}}catch(n){return{kind:"err",reason:n instanceof Error?n.message:"network"}}}var se=new Set(["the","a","an","to","of","on","in","and","or","section","button","link","card","tile","now"]),ce=[{keyword:"button",matchTag:/^(button)$/i,matchRole:"button"},{keyword:"link",matchTag:/^(a)$/i,matchRole:"link"},{keyword:"card",matchTag:/^(article|div|section)$/i},{keyword:"section",matchTag:/^(section|main|article)$/i}],le=.4;function w(e){let n=I(e);if(n.size===0)return null;let t=de(document.body),o=null;for(let a of t){if(!a.visible)continue;let i=me(a,e,n);i<le||(!o||i>o.score)&&(o={c:a,score:i})}return o?.c.element??null}function I(e){return new Set(e.toLowerCase().replace(/[^a-z0-9 ]/g," ").split(/\s+/).filter(n=>n.length>0&&!se.has(n)))}function de(e){let n=[],t=document.createTreeWalker(e,NodeFilter.SHOW_ELEMENT),o=t.nextNode();for(;o;){if(o instanceof HTMLElement){let a=pe(o);a&&n.push({element:o,role:o.getAttribute("role")??o.tagName.toLowerCase(),name:a,visible:ue(o)})}o=t.nextNode()}return n}function pe(e){let n=e.getAttribute("aria-labelledby");if(n){let i=n.split(/\s+/).map(r=>document.getElementById(r)?.textContent?.trim()??"").filter(Boolean);if(i.length>0)return i.join(" ")}let t=e.getAttribute("aria-label");if(t)return t.trim();if(e.id){let i=document.querySelector(`label[for="${ge(e.id)}"]`);if(i?.textContent)return i.textContent.trim()}let o=e.getAttribute("alt")??e.getAttribute("title");if(o)return o.trim();let a=(e.textContent??"").trim();return a&&a.length<200?a:""}function ue(e){if(!e.isConnected)return!1;let n=e.ownerDocument.defaultView?.getComputedStyle(e);return n?!(n.display==="none"||n.visibility==="hidden"||n.opacity==="0"):!0}function me(e,n,t){let o=I(e.name);if(o.size===0)return 0;let a=0;for(let u of t)o.has(u)&&a++;let i=new Set([...t,...o]).size,r=i===0?0:a/i,s=0,c=n.toLowerCase();for(let u of ce)if(c.includes(u.keyword)&&(u.matchTag.test(e.element.tagName)||e.role===u.matchRole)){s=.15;break}let d=e.element.getAttribute("data-tour-stop"),p=0;if(d){let u=I(d.replace(/-/g," ")),y=0;for(let T of t)u.has(T)&&y++;y>0&&(p=.5*(y/t.size))}return Math.min(1,r+s+p)}function ge(e){return e.replace(/(["\\])/g,"\\$1")}var fe="data-shoppingmate-pulse-ring";function R(e,n){let t=e.getBoundingClientRect(),o=document.createElement("div");o.setAttribute(fe,""),Object.assign(o.style,{position:"fixed",left:`${t.left-6}px`,top:`${t.top-6}px`,width:`${t.width+12}px`,height:`${t.height+12}px`,borderRadius:"14px",boxShadow:"0 0 0 3px rgba(139,92,246,0.85), 0 0 24px rgba(139,92,246,0.55)",pointerEvents:"none",zIndex:"2147483646",animation:"shoppingmate-pulse 1.2s ease-in-out infinite"}),he(),document.body.appendChild(o);let a=!1,i=()=>{a||(a=!0,o.remove())};return setTimeout(i,n),i}var P=!1;function he(){if(P)return;P=!0;let e=document.createElement("style");e.textContent=`@keyframes shoppingmate-pulse {
    0%, 100% { transform: scale(1); opacity: 1; }
    50% { transform: scale(1.04); opacity: 0.85; }
  }`,document.head.appendChild(e)}async function O(e){switch(e.type){case"navigate":return ye(e.path);case"scroll_to":return be(e.intent);case"highlight":return ve(e.intent,e.durationMs??2e3);case"click":return xe(e.intent)}}function ye(e){try{let n=new URL(e,window.location.href);return n.origin!==window.location.origin?{ok:!1,reason:"cross_origin"}:(window.location.assign(n.pathname+n.search+n.hash),{ok:!0})}catch{return{ok:!1,reason:"route_not_found"}}}function be(e){let n=w(e);return n?(n.scrollIntoView({behavior:"smooth",block:"center"}),{ok:!0}):{ok:!1,reason:"not_found"}}function ve(e,n){let t=w(e);return t?(R(t,n),{ok:!0}):{ok:!1,reason:"not_found"}}function xe(e){let n=w(e);return n?n.isConnected?(n.click(),{ok:!0}):{ok:!1,reason:"stale_target"}:{ok:!1,reason:"not_found"}}var U={"calm-clinician":"Sage",stylist:"Lumi",coach:"Kai",concierge:"Olivia",curator:"Theo",guide:"Maya",expert:"Arjun",host:"Ana"},ke="concierge";function we(){let e="https://shoppingmate-web.vercel.app/widget/personas";return e&&typeof e=="string"?e.replace(/\/$/,""):"https://cdn.shoppingmate.ai/v1/personas"}function C(e){let n=e&&U[e]?e:ke,t=U[n]??"Olivia";return{id:n,name:t,initial:t.charAt(0).toUpperCase(),avatarUrl:`${we()}/${n}.png`}}var V=0,g=()=>(V+=1,`t${V}`);function Se(e,n){switch(n.type){case"set_mode":return{...e,mode:n.mode};case"set_voice_state":return{...e,voiceState:n.state};case"set_connection":return{...e,connection:n.status};case"reset":return{...e,transcript:[],thinking:!1,closed:!1,closedReason:null,checkoutUrl:null,capWarning:null};case"user_input":return{...e,transcript:[...e.transcript,{id:g(),role:"user",kind:"text",text:n.text,ts:Date.now()}]};case"agent_event":{let t=n.event;switch(t.type){case"thinking":return{...e,thinking:!0};case"end_of_turn":return{...e,thinking:!1};case"say":{let o=e.transcript[e.transcript.length-1];return o&&o.role==="agent"&&o.kind==="text"&&o.partial?{...e,thinking:!1,transcript:[...e.transcript.slice(0,-1),{...o,text:t.text,partial:!1,ts:Date.now()}]}:{...e,thinking:!1,transcript:[...e.transcript,{id:g(),role:"agent",kind:"text",text:t.text,ts:Date.now()}]}}case"say_partial":{let o=e.transcript[e.transcript.length-1];return o&&o.role==="agent"&&o.kind==="text"&&o.partial?{...e,thinking:!1,transcript:[...e.transcript.slice(0,-1),{...o,text:t.text,ts:Date.now()}]}:{...e,thinking:!1,transcript:[...e.transcript,{id:g(),role:"agent",kind:"text",text:t.text,ts:Date.now(),partial:!0}]}}case"user_text":return{...e,transcript:[...e.transcript,{id:g(),role:"user",kind:"text",text:t.text,ts:Date.now()}]};case"cards":return{...e,transcript:[...e.transcript,{id:g(),role:"agent",kind:"cards",items:t.items,ts:Date.now()}]};case"tool_result":return e;case"checkout_redirect":return{...e,checkoutUrl:t.url};case"cap_warning":return{...e,capWarning:{reason:t.reason,remaining:t.remaining},transcript:[...e.transcript,{id:g(),role:"system",kind:"cap_warning",remaining:t.remaining,ts:Date.now()}]};case"session_closed":return{...e,closed:!0,closedReason:t.reason,transcript:[...e.transcript,{id:g(),role:"system",kind:"closed",reason:t.reason,ts:Date.now()}]};default:return e}}default:return e}}function E(e){let n={sessionId:e.sessionId,mode:"pill",voiceState:"idle",transcript:[],thinking:!1,closed:!1,closedReason:null,checkoutUrl:null,capWarning:null,connection:"connecting"},t=[];return{get:()=>n,dispatch:o=>{n=Se(n,o);for(let a of t)a(n)},subscribe:o=>(t.push(o),()=>{let a=t.indexOf(o);a>=0&&t.splice(a,1)})}}var D=`
:host { all: initial; }
...
`;function _e(e){if(!e||typeof e.type!="string")return!1;switch(e.type){case"navigate":return typeof e.path=="string";case"scroll_to":case"highlight":case"click":return typeof e.intent=="string";default:return!1}}function v(e){return JSON.stringify(e)}function A(e){let n;try{n=JSON.parse(e)}catch{return null}if(!n||typeof n!="object")return null;let t=n;switch(t.type){case"thinking":return{type:"thinking"};case"say":return typeof t.text=="string"?{type:"say",text:t.text}:null;case"say_partial":return typeof t.text=="string"?{type:"say_partial",text:t.text}:null;case"user_text":return typeof t.text=="string"?{type:"user_text",text:t.text}:null;case"cards":return Array.isArray(t.items)?{type:"cards",items:t.items}:null;case"tool_result":return typeof t.toolName!="string"||typeof t.ok!="boolean"?null:{type:"tool_result",toolName:t.toolName,ok:t.ok,summary:typeof t.summary=="string"?t.summary:void 0};case"checkout_redirect":return typeof t.url=="string"?{type:"checkout_redirect",url:t.url}:null;case"cap_warning":return t.reason!=="turns"&&t.reason!=="voice_ms"&&t.reason!=="duration_ms"||typeof t.remaining!="number"?null:{type:"cap_warning",reason:t.reason,remaining:t.remaining};case"end_of_turn":return{type:"end_of_turn"};case"session_closed":return t.reason!=="user"&&t.reason!=="cap"&&t.reason!=="error"?null:{type:"session_closed",reason:t.reason};case"host_action_request":{if(typeof t.callId!="string"||!t.action)return null;let o=t.action;return _e(o)?{type:"host_action_request",callId:t.callId,action:o}:null}case"persona_swap":return typeof t.personaId=="string"?{type:"persona_swap",personaId:t.personaId}:null;default:return null}}var B=[1e3,2e3,4e3,8e3,16e3],Me=5;function W(e,n){let t=null,o=0,a=!1,i=[];function r(){a||(n.onStatus(o>0?"reconnecting":"connecting"),t=new WebSocket(e),t.onopen=()=>{n.onStatus("connected"),o>0&&t?.send(JSON.stringify({type:"session_resume",sessionId:n.sessionId})),o=0;for(let s of i)t?.send(s);i=[]},t.onmessage=s=>n.onEvent(typeof s.data=="string"?s.data:""),t.onerror=()=>{},t.onclose=()=>{if(a)return;if(o+=1,o>=Me){n.onStatus("disconnected");return}let s=Math.min(o-1,B.length-1),c=B[s]??3e4;n.onStatus("reconnecting"),setTimeout(r,c)})}return r(),{send:s=>{t&&t.readyState===1?t.send(s):i.push(s)},close:()=>{a=!0,t?.close()}}}var l={trayConnected:"CONNECTED",trayOffline:"OFFLINE",micStart:"Start voice call",micMute:"Mute mic",micUnmute:"Unmute mic",endCallAria:"End call",closeAria:"Close",openAria:"Open shoppingmate",panelHelpHeading:"Hi, I'm",panelHelpSubtitle:"I'm here to help you:",panelBullets:["Find the right product fast","Compare options out loud","Check out without leaving the page"],poweredBy:"Powered by shoppingmate",chatPlaceholder:"Type a quick question\u2026",reconnecting:"Reconnecting\u2026",disconnected:"Connection lost \u2014 reload to retry",closed:{user:"Conversation ended",cap:"Time to wrap up \u2014 reload for a new chat",error:"Something went wrong"},payNow:"Pay now \u2192",capWarning:"A couple minutes left",thinking:"thinking\u2026",micDenied:"Mic blocked \u2014 switching to text"};var f=e=>`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${e}</svg>`,ct=f('<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>'),S=f('<path d="M18 6 6 18"/><path d="m6 6 12 12"/>'),lt=f('<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>'),z=f('<path d="M10.68 13.31a16 16 0 0 0 3.41 2.6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7 2 2 0 0 1 1.72 2v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.42 19.42 0 0 1-3.33-2.67"/><path d="M5 5a2 2 0 0 1 2-2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L10.9 11.1"/><line x1="22" y1="2" x2="2" y2="22"/>'),F=f('<path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="22"/>'),j=f('<line x1="2" y1="2" x2="22" y2="22"/><path d="M18.89 13.23A7.12 7.12 0 0 0 19 12v-2"/><path d="M5 10v2a7 7 0 0 0 12 5"/><path d="M15 9.34V5a3 3 0 0 0-5.68-1.33"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12"/><line x1="12" y1="19" x2="12" y2="22"/>'),K=f('<path d="M22 2 11 13"/><path d="M22 2l-7 20-4-9-9-4 20-7z"/>');function _(e){return e.replace(/[&<>"']/g,n=>n==="&"?"&amp;":n==="<"?"&lt;":n===">"?"&gt;":n==='"'?"&quot;":"&#39;")}function Te(e,n){let t=document.createElement("button");return t.className="card",t.type="button",t.dataset.sku=e.sku,t.innerHTML=`
  ...
  `,t.addEventListener("click",()=>n({sku:e.sku,variantId:e.variantId})),t}function Ie(e,n){if(e.kind==="text"){let o=document.createElement("div");return o.className=`bubble ${e.role}`,o.textContent=e.text,o}if(e.kind==="cards"){let o=document.createElement("div");o.className="cards-row";for(let a of e.items)o.appendChild(Te(a,n));return o}if(e.kind==="cap_warning"){let o=document.createElement("div");return o.className="bubble system",o.textContent=l.capWarning,o}let t=document.createElement("div");return t.className="bubble system",t.textContent=l.closed[e.reason],t}var q=new WeakMap;function M(e,n,t){let o=q.get(e)??[],a=new Map(o.map(c=>[c.id,c])),i=new Set(n.map(c=>c.id));for(let c of o)i.has(c.id)||c.el.remove();let r=[],s=!1;for(let c=0;c<n.length;c++){let d=n[c];if(!d)continue;let p=a.get(d.id);if(p)d.kind==="text"&&p.text!==d.text&&(p.el.textContent=d.text,p.text=d.text,s=!0),r.push(p);else{let u=Ie(d,t);e.appendChild(u),r.push({id:d.id,el:u,text:d.kind==="text"?d.text:void 0}),s=!0}}q.set(e,r),s&&(e.scrollTop=e.scrollHeight)}function G(e){return e.muted?"you're muted":e.voiceState==="speaking"?`${e.personaName} is speaking\u2026`:`${e.personaName} is listening\u2026`}function Ce(e){return`${e.checkoutUrl??""}|${e.personaName}`}function Y(e,n){let t=Ce(n);e.dataset.chromeKey!==t&&(e.innerHTML=`
    ...
    `,e.querySelector('[data-action="close"]')?.addEventListener("click",n.onClose),e.querySelector('[data-action="checkout"]')?.addEventListener("click",n.onCheckout),e.dataset.chromeKey=t);let o=e.querySelector('[data-region="status"]');if(o instanceof HTMLElement){let i=G(n);o.textContent!==i&&(o.textContent=i)}let a=e.querySelector('[data-region="transcript"]');a instanceof HTMLElement&&M(a,n.transcript,n.onCardTap)}function Ee(e){return`${e.transcript.length===0?"1":"0"}|${e.checkoutUrl??""}|${e.closed?"1":"0"}|${e.personaName}|${e.personaInitial}|${e.personaAvatarUrl}`}function J(e,n){let t=Ee(n);if(e.dataset.chromeKey!==t){let a=n.transcript.length===0,i=l.panelBullets.map(d=>`<li>${d}</li>`).join(""),r=a?`
        <div class="welcome">
  ...
  `,o.querySelector('[data-action="accept"]')?.addEventListener("click",n),o.querySelector('[data-action="dismiss"]')?.addEventListener("click",t),e.appendChild(o),o}var Z="shoppingmate-widget";function Ne(){return"live-kit"==="web-speech"?"web-speech":"live-kit"}var H=class extends HTMLElement{constructor(){super(...arguments);m(this,"rootEl",null);m(this,"pillHost",null);m(this,"panelHost",null);m(this,"store",E({sessionId:"pending"}));m(this,"socket",null);m(this,"voiceMode",h(null,x()));m(this,"voice",null);m(this,"persona",C(null));m(this,"apiBase","");m(this,"merchantId","");m(this,"domain",window.location.host)}connectedCallback(){if(this.shadowRoot)return;let t=this.getAttribute("data-id"),o=this.getAttribute("data-api")??this.apiBase;if(!t){console.warn("[shoppingmate] data-id missing on widget element");return}this.merchantId=t,this.apiBase=o;let a=this.attachShadow({mode:"open"}),i=document.createElement("style");i.textContent=D,a.appendChild(i);let r=document.createElement("div");r.className="root",a.appendChild(r),this.rootEl=r,this.panelHost=document.createElement("div"),this.pillHost=document.createElement("div"),r.appendChild(this.panelHost),r.appendChild(this.pillHost),this.store.subscribe(()=>this.render()),this.render(),this.start()}disconnectedCallback(){this.socket?.close(),this.voiceMode.stop()}async start(){let t=await $({apiBase:this.apiBase,merchantId:this.merchantId,domain:this.domain});if(t.kind==="err"){console.warn("[shoppingmate] bootstrap failed:",t.reason);return}this.store=E({sessionId:t.sessionId}),this.store.subscribe(()=>this.render()),this.voice=t.voice,this.persona=C(t.voice?.personaId??null);let o=Ne(),a=b();if(o==="live-kit"&&this.voice){let r=k({stack:"live-kit",livekit:{wsUrl:this.voice.wsUrl,token:this.voice.token,roomName:this.voice.roomName,onTranscriptEvent:s=>this.handleLiveKitData(s)}});r&&(this.voiceMode=r)}else{let r=k({stack:"web-speech"});r&&(this.voiceMode=r),a?.onFinal(s=>{this.store.dispatch({type:"user_input",text:s,mode:"voice"}),this.socket?.send(v({type:"user_text",sessionId:t.sessionId,text:s,mode:"voice"}))})}this.voiceMode.onStateChange(r=>this.store.dispatch({type:"set_voice_state",state:r})),this.socket=W(t.wsUrl,{sessionId:t.sessionId,onEvent:r=>{let s=A(r);s&&this.handleAgentEvent(s)},onStatus:r=>this.store.dispatch({type:"set_connection",status:r})}),this.merchantId==="SM-XPK2EN"&&Q(document.body,{onAccept:()=>{this.publishWidgetMessage({type:"tour_request"}),this.openCall()},onDismiss:()=>{}})}async handleAgentEvent(t){if(t.type==="host_action_request"){let o=await O(t.action);this.publishWidgetMessage({type:"host_action_result",callId:t.callId,result:o});return}t.type!=="persona_swap"&&(this.store.dispatch({type:"agent_event",event:t}),t.type==="say"&&this.voiceMode.speak(t.text))}publishWidgetMessage(t){this.socket?.send(v(t))}render(){if(!this.pillHost||!this.panelHost)return;let t=this.store.get(),o=b()!==null;t.mode==="call"?Y(this.panelHost,{voiceState:t.voiceState,muted:t.voiceState==="muted",transcript:t.transcript,checkoutUrl:t.checkoutUrl,personaName:this.persona.name,onClose:()=>this.store.dispatch({type:"set_mode",mode:"pill"}),onCardTap:a=>this.cardTap(a),onCheckout:()=>{}}):t.mode==="chat"||t.mode==="expanded"?J(this.panelHost,{transcript:t.transcript,checkoutUrl:t.checkoutUrl,personaName:this.persona.name,personaInitial:this.persona.initial,personaAvatarUrl:this.persona.avatarUrl,onSend:a=>this.userText(a,"text"),onCall:()=>this.openCall(),onClose:()=>this.store.dispatch({type:"set_mode",mode:"pill"}),onCardTap:a=>this.cardTap(a),closed:t.closed}):this.panelHost.innerHTML="",X(this.pillHost,{mode:t.mode,callable:o,voiceState:t.voiceState,personaName:this.persona.name,personaInitial:this.persona.initial,personaAvatarUrl:this.persona.avatarUrl,onCall:()=>this.openCall(),onMute:a=>this.voiceMode.setMuted(a),onEnd:()=>{this.voiceMode.stop(),this.store.dispatch({type:"set_mode",mode:"pill"})},onChat:()=>this.store.dispatch({type:"set_mode",mode:"chat"}),onClose:()=>this.store.dispatch({type:"set_mode",mode:"pill"})})}openCall(){this.store.dispatch({type:"set_mode",mode:"call"}),this.voiceMode.start()}userText(t,o){this.store.dispatch({type:"user_input",text:t,mode:o});let a=this.store.get().sessionId;this.socket?.send(v({type:"user_text",sessionId:a,text:t,mode:o}))}handleLiveKitData(t){let o;try{o=new TextDecoder().decode(t)}catch{return}let a=A(o);a&&this.handleAgentEvent(a)}cardTap(t){let o=this.store.get().sessionId;this.socket?.send(v({type:"card_tap",sessionId:o,action:"cartAdd",sku:t.sku,variantId:t.variantId,qty:1}))}};function ee(){customElements.get(Z)||customElements.define(Z,H)}function $e(){let e=document.currentScript instanceof HTMLScriptElement?document.currentScript:null,n=e?.dataset.id;if(!n){console.warn("[shoppingmate] data-id missing on script tag");return}if(document.querySelector("shoppingmate-widget"))return;ee();let t=document.createElement("shoppingmate-widget");t.setAttribute("data-id",n);let o=e?.dataset.api;t.setAttribute("data-api",o??"https://api-production-1ea1.up.railway.app"),document.body?document.body.appendChild(t):document.addEventListener("DOMContentLoaded",()=>document.body.appendChild(t),{once:!0})}$e();})();
