...
| ----------------------------- | ------------- | ------------------------------------------ | ---- |
| 1 — Working widget end-to-end | In progress   | `2026-04-30-shoppingmate-phase1-design.md` | Plans 1–3 ✅ complete; Plans 4–7 pending |
...
**Plans 1–3 complete (2026-05-04).** Repo-wide: `pnpm typecheck` clean across all 8 workspaces; `pnpm test` 45 files / 230 tests passing; `pnpm lint` 4 pre-existing errors in quarantined slack workstream (not Plan 3 work). Skipped per instructions: live dev-store acceptance runs and `git tag` tasks.

**Phase 1 is NOT closed yet** — Plans 1–3 built the adapter substrate (catalog + cart + smoke). The agent runtime, the widget, the voice stack, and conversion attribution are still to come. Plans 4–7 below close Phase 1.

### Phase 1 closing plans (2026-05-04 — pending brainstorm + writing-plans)

| Plan                                                                         | Status        | Scope summary |
| ---------------------------------------------------------------------------- | ------------- | ------------- |
| Plan 4 — Backend agent runtime (Sonnet 4.6 tool-use loop)                    | 🟡 Brainstorm | Sonnet 4.6 tool-use loop wrapping the 8 adapters. Mode-agnostic stream of `{kind:"say", text}` and `{kind:"cards", items[]}` events — voice speaks the text, widget renders product cards (image + title + price + variantId + productUrl). Tap-on-card and voice ("add the second one") both emit the same `cartAdd` tool call. Per-conversation hard caps (15 turns / 3 min voice / 25 min duration — §4 Phase 3 margin guarantee). |
| Plan 5 — Voice-first widget shell (gtag.js bundle, simulated audio)          | 🟡 Brainstorm | Real `cdn.shoppingmate.ai/v1.js` Web Component matching `web/src/components/WidgetPreview.tsx`: floating pill → expanded → call/chat. Call mode = waveform + heterogeneous live transcript (text bubbles **and** product cards with images, inline). Chat = "text fallback · voice preferred". Browser STT/TTS for now to validate UX end-to-end before committing to LiveKit. Wires through the existing JWT-gated WS to Plan 4's runtime. |
| Plan 6 — Voice stack (LiveKit Cloud + Gemini 2.5 Flash Live native audio)    | 🟡 Brainstorm | Per [ADR-0001](../adr/2026-05-01-voice-stack-livekit-gemini-live.md): swap Plan 5's simulated audio for real WebRTC + Gemini Live native audio. 8 personas via voice-descriptor prompts. Voice agent **never speaks numeric prices** (paraphrase + defer to screen — §4 Phase 1 invariant). |
